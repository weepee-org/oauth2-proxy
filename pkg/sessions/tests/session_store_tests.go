	...
	Describe("Session Store Suite", func() {
		...
		Context("with a cipher", func() {
			BeforeEach(func() {
				...
				Expect(err).ToNot(HaveOccurred())
				input.cookieOpts.Secret = base64.URLEncoding.EncodeToString(secret)
				cipher, err := encryption.NewCipher(encryption.SecretBytes(input.cookieOpts.Secret))
				Expect(err).ToNot(HaveOccurred())
	...
	Context("when Save is called", func() {
		...
		Context("with a broken session", func() {
			BeforeEach(func() {
				...
				value := encryption.SignedValue(in.cookieOpts.Secret, in.cookieOpts.Name, broken, time.Now())
...
func LoadSessionTests(in *testInput) {
	...
	It("loads a session equal to the original session", func() {
		if in.cookieOpts.Secret == "" {
			// Only Email and User stored in session when encrypted
			Expect(loadedSession.Email).To(Equal(in.session.Email))
			Expect(loadedSession.User).To(Equal(in.session.User))
		} else {
