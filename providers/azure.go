...
func NewAzureProvider(p *ProviderData) *AzureProvider {
	p.setProviderDefaults(azureProviderName, azureDefaultLoginURL, azureDefaultRedeemURL, azureDefaultProfileURL, nil, azureDefaultScope)
...
func (p *AzureProvider) Configure(tenant string) {
	...
	p.Tenant = tenant
	if p.LoginURL == nil || p.LoginURL.String() == "" || p.LoginURL.String() == azureDefaultLoginURL.String() {
		p.LoginURL = &url.URL{
			Scheme: "https",
			Host:   "login.microsoftonline.com",
			Path:   "/" + p.Tenant + "/oauth2/authorize"}
	}
	if p.RedeemURL == nil || p.RedeemURL.String() == "" || p.RedeemURL.String() == azureDefaultRedeemURL.String() {
		p.RedeemURL = &url.URL{
			Scheme: "https",
			Host:   "login.microsoftonline.com",
